...
- **举报系统**
  ...
  - 后台管理员可以审核举报，支持以下操作：
    ...
    - 撤销之前的处理操作
  - 审核完成后自动向举报人发送消息通知
  - 审核完成后自动向被举报人发送警告消息（红色提醒）
  - 举报成功奖励积分（可在后台配置）

- **软删除功能**
  - 删除操作改为软删除（设置is_deleted=1），内容可恢复
  - 漂流瓶和评论都支持软删除
  - 撤销处理时可以恢复被删除的内容

- **消息通知系统增强**
  - 新增"举报反馈"消息类型
  - 系统消息支持（from_user_id为NULL）
  - 被举报人收到红色警告消息，提醒内容被处理的原因
  - 举报人收到蓝色信息消息，告知举报处理结果

### 数据库更新

- 新增 `reports` 表（举报表）
- `bottles` 表新增字段：
  - `is_hidden` (TINYINT(1)): 是否屏蔽
  - `is_deleted` (TINYINT(1)): 是否删除（软删除）
- `comments` 表新增字段：
  - `is_hidden` (TINYINT(1)): 是否屏蔽
  - `is_deleted` (TINYINT(1)): 是否删除（软删除）
- `messages` 表更新：
  - `from_user_id` 字段允许 NULL（表示系统消息）
  - `type` 字段新增 '举报反馈' 类型
- `system_settings` 表新增配置：
  - `POINTS_PER_REPORT_APPROVED`: 举报成功奖励积分（默认5）

### 前端更新

- 在 `pick.html`、`profile_picked.html`、`profile_bottles.html` 添加举报按钮
- 举报模态框支持选择默认理由或自定义理由
- 消息中心支持显示系统消息和举报反馈消息
- 被举报人警告消息使用红色警告样式显示
...
- 新增"举报管理"页面（`admin/reports.php`）
- 支持查看待审核、已审核、已拒绝的举报
- 支持批量处理同一内容的多个举报
- 支持撤销之前的处理操作
- 系统设置页面新增举报成功积分配置

## 版本 (v1.3.1) - 2024年12月20日
