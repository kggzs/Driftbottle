...
class EpayCore
{
    ...
    public function pagePay($param_tmp, $button='正在跳转'){
        ...
        $html = '<form id="dopay" action="'.htmlspecialchars($requrl, ENT_QUOTES).'" method="post">';
        foreach ($param as $k=>$v) {
            $html.= '<input type="hidden" name="'.htmlspecialchars($k, ENT_QUOTES).'" value="'.htmlspecialchars($v, ENT_QUOTES).'"/>';
        }
        $html .= '<input type="submit" value="'.htmlspecialchars($button, ENT_QUOTES).'"></form>';
        // 注意：不包含script标签，因为通过innerHTML插入的script不会执行
        // 前端需要手动调用form.submit()
